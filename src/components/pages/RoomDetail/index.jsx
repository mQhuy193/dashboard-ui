...
const RoomDetail = () => {
  ...
  const nextImage = () => {
    if (room?.media && room.media.length > 0) {
      setCurrentImageIndex((prev) => 
        prev === room.media.length - 1 ? 0 : prev + 1
      );
    }
  };
  ...
  const prevImage = () => {
    if (room?.media && room.media.length > 0) {
      setCurrentImageIndex((prev) => 
        prev === 0 ? room.media.length - 1 : prev - 1
      );
    }
  };
  ...
  const imageList = room.media?.filter(m => 
    m.mediaUrl && /\.(png|jpg|jpeg|gif|webp)$/i.test(m.mediaUrl)
  ) || [];
